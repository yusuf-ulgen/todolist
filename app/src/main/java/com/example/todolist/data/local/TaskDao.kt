...
interface TaskDao {
    ...
    @Query("SELECT * FROM tasks WHERE userId = :uid ORDER BY isPinned DESC, sortOrder ASC")
    suspend fun getAllTasks(uid: String): List<Task>

    @Query("SELECT * FROM tasks")
    suspend fun getAllTasksUnfiltered(): List<Task>
